...
# Install Redis drop-in config.
# Ubuntu's Redis supports includedir; use it if present, else append include.
REDIS_DROPIN_DIR="/etc/redis/conf.d"
REDIS_DROPIN="${REDIS_DROPIN_DIR}/10-app.conf"
REDIS_MAIN="/etc/redis/redis.conf"

mkdir -p "${REDIS_DROPIN_DIR}"
export REDIS_PASS
render_template \
    "${SCRIPT_DIR}/config/redis-app.conf.tmpl" \
    "${REDIS_DROPIN}" \
    "640" "redis:redis"

# Add includedir directive to redis.conf if not already there
if ! grep -q "^includedir ${REDIS_DROPIN_DIR}" "${REDIS_MAIN}" 2>/dev/null; then
    echo "" >> "${REDIS_MAIN}"
    echo "includedir ${REDIS_DROPIN_DIR}" >> "${REDIS_MAIN}"
fi

systemctl enable --now redis-server
systemctl restart redis-server
success "Redis configured with authentication"
...
echo ""
echo -e "${NC}"
info "Secrets (DB password, Redis password, etc.) are stored in:"
